...
const actualizarCantidadResumen = async (servicioId, event) => {
  ...
  if (isNaN(nuevaCantidad) || nuevaCantidad < 1) {
    // No hacer nada si la cantidad no es válida
    return
  }
...
// Cargar los datos del carrito al montar el componente
onMounted(() => {
  fetchCartData()
})
</script>
  ...
  <header class="fixed w-full top-0 z-50 bg-white/90 shadow-md transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4">
      <nav class="flex justify-between items-center py-4">
        <!-- Logo -->
        <RouterLink to="/" class="flex items-center text-violet-600 font-bold text-2xl no-underline hover:text-red-400">
        ...
        <ul class="hidden md:flex space-x-8 items-center">
          <li>
            <RouterLink to="/" :class="clasesRuta['/']">Inicio</RouterLink>
          </li>
          <li>
            <RouterLink to="/servicios" :class="clasesRuta['/servicios']">Servicios</RouterLink>
          </li>
          <li>
            <RouterLink to="/sobre-nosotros" :class="clasesRuta['/sobre-nosotros']">Sobre Nosotros</RouterLink>
          </li>

          <!-- Ícono de Carrito (visible solo si está autenticado) -->
          <li v-if="usuario" class="relative h-full flex items-center" @mouseenter="handleMouseEnterCart"
            @mouseleave="handleMouseLeaveCart">
            <RouterLink to="/cart"
              class="text-gray-900 hover:text-violet-600 transition-colors duration-300 relative p-2 rounded-full hover:bg-gray-100">
              🛒 Carrito
            ...
            <!-- Resumen del carrito -->
            <div v-if="mostrarResumenCarrito"
              class="absolute right-0 top-full mt-0 w-80 bg-white rounded-lg shadow-lg py-2 z-50 border border-gray-200">
                ...
                <li v-for="item in cartState.items" :key="item.servicioId._id"
                  ...
                  <div class="flex-1">
                    ...
                    <div class="flex items-center mt-2">
                      <label for="cantidad" class="text-gray-600 text-xs mr-2">Cantidad:</label>
                      <input type="number" :value="item.cantidad"
                        @change="actualizarCantidadResumen(item.servicioId._id, $event)" min="1"
                      ...
                      <button @click="quitarItemResumen(item.servicioId._id)"
                        class="ml-3 text-red-500 hover:text-red-700 text-sm focus:outline-none">
                        Eliminar
                      </button>
                    </div>
                  </div>
              ...
              <div class="px-4 py-2 text-right font-bold text-violet-800 border-t border-gray-200 text-lg">
          ...
          <li v-else class="relative">
            <button @click="toggleMenu"
              class="flex items-center gap-2 text-gray-800 hover:text-violet-600 transition-colors">
            ...
            <div v-if="mostrarMenu"
              ...
              <button @click="irAEditarPerfil"
                class="w-full text-left px-4 py-2 text-gray-700 hover:bg-violet-50 hover:text-violet-600 transition-colors">
                Editar Perfil
              </button>
